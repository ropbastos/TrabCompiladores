...
void asm_after(asm_inst_list_item* prev_item, asm_inst* new_inst)
{
  ...
  // Allocate new item.
  asm_inst_list_item* new_item = malloc(sizeof(inst_list_item));
...
asm_inst_list_item* optimize(asm_inst_list_item* asm_code)
{
  asm_inst_list_item* head = asm_code;
  asm_inst_list_item* current = asm_code;
  int not_in_call = 1; /* Since the return address is calculated from the 
                        number of instructions on the call sequence, 
                        those can't be changed. */
  while (current != NULL)
  {
    if (current->inst->op && !strcmp(current->inst->op, "lea"))
    {
      not_in_call = 0;
    }
    else if (current->inst->op && !strcmp(current->inst->op, "jmp"))
    {
      not_in_call = 1;
    }
    if (not_in_call)
    {
      ...
      {
        // Clear subsequent writes to the same place.
        if (!strcmp(current->inst->dst, current->next->inst->dst))
        {
          current = current->next;
          asm_del(&head, current->prev);
        }

        // Eliminate intermediaries when passing values.
        if (!strcmp(current->inst->dst, current->next->inst->src)
            && !(current->inst->lbl || current->next->inst->lbl)
            && !(current->inst->src[0] == '-' && current->next->inst->dst[0] == '-')
            && current->inst->dst[0] != '-' && current->next->next 
            && not_in_call)
        {
          char* src = current->inst->src;
          char* dst = current->next->inst->dst;
          asm_inst* direct_transfer = asm_op(NULL, "movq", src, dst);
          asm_after(current->next, direct_transfer);

          asm_inst_list_item* del1 = current;
          asm_inst_list_item* del2 = current->next;
